...
SumTable <- function(res){
  ...
  tmp1 <- data.frame("Ave.A" = apply(a,1,mean),
                     "Ave.B" = apply(b,1,mean),
                     "Ave.C" = apply(c,1,mean),
                     "Ave.D" = apply(d,1,mean))
  
  tmp2 <- data.frame("Stdev.A" = apply(a,1,sd),
                     "Stdev.B" = apply(b,1,sd),
                     "Stdev.C" = apply(c,1,sd),
                     "Stdev.D" = apply(d,1,sd))
  
  tmp3 <- tmp2/tmp1
  colnames(tmp3) <- c("CV.A","CV.B","CV.C","CV.D")
  
  tmp4 <- data.frame("FC.B.A" = tmp1$Ave.B / tmp1$Ave.A,
                     "FC.C.A" = tmp1$Ave.C / tmp1$Ave.A,
                     "FC.D.A" = tmp1$Ave.D / tmp1$Ave.A)
  ...
  tmp5 <- data.frame("p.B.A" = apply(cbind(a,b),1,getp),
                     "p.C.A" = apply(cbind(a,c),1,getp),
                     "p.D.A" = apply(cbind(a,d),1,getp))

  rownames(tmp4) <- rownames(res)
  
  tmp6 <- data.frame("adj.p.B.A" = p.adjust(tmp5$p.B.A,method = "BH"),
                     "adj.p.C.A" = p.adjust(tmp5$p.C.A,method = "BH"),
                     "adj.p.D.A" = p.adjust(tmp5$p.D.A,method = "BH"),
                     "name" = gsub(".*:","",rownames(res)),
                     "species" = gsub(".*_","",rownames(res)),stringsAsFactors = F)
  
  tmp7 <- data.frame("ori.FC.B.A" = orifc(tmp6$species,human = 1,ecoli = 1.5, yeast = 0.9),
                     "ori.FC.C.A" = orifc(tmp6$species,human = 1,ecoli = 2, yeast = 0.8),
                     "ori.FC.D.A" = orifc(tmp6$species,human = 1,ecoli = 2.5, yeast = 0.7)
                     )
  
  return(cbind(tmp1,tmp3,tmp4,tmp5,tmp6,tmp7))
...
getp <- function(res2){
  myTtest <- try(t.test(res2[1:8],res2[9:16],var.equal = T,conf.level=0.95))
